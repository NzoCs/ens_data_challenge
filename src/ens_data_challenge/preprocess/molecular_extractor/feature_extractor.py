...
class MolecularFeatureExtractor:
    """
    Extracteur de features moléculaires enrichir
    """

    def __init__(
        self,
        cosmic_loader: COSMICDatabaseLoader,
        clinvar_annotator: ClinVarAnnotator,
        conservation_scorer: ConservationScorer,
        domain_annotator: ProteinDomainAnnotator,
    ) -> None:
        # Bases de données
    ...
    def extract_cosmic_features(
        self, chr_str: str, start: int, ref: str, alt: str, gene: str
    ) -> Dict:
    ...
    def extract_clinvar_features(
        self, chr_str: str, start: int, ref: str, alt: str
    ) -> Dict:
        """Features ClinVar"""
        features = {}
    ...
    def extract_domain_features(self, gene: str, aa_position: Optional[int]) -> Dict:
        ...
        if domain:
            ...
            features["domain_importance_score"] = importance_scores.get(
                domain["importance"], 0.5
            )

            # Type de domaine
            features["in_catalytic_domain"] = (
                1 if domain["type"] in ["catalytic", "kinase"] else 0
            )
            features["in_binding_domain"] = (
                1 if "binding" in domain["type"].lower() else 0
            )
            features["in_structural_domain"] = (
                1 if domain["type"] == "structural" else 0
            )
    ...
    def extract_positional_features(self, chr_str: str, start: int, end: int) -> Dict:
        ...
        features["distance_to_centromere_normalized"] = (
            features["distance_to_centromere"] / chr_len
        )

        # Régions spéciales
        telomere_threshold = 5000000
        features["is_telomeric"] = (
            1
            if (start < telomere_threshold or start > chr_len - telomere_threshold)
            else 0
        )

        centromere_region = (0.4 * chr_len, 0.6 * chr_len)
        features["is_centromeric"] = (
            1 if centromere_region[0] < start < centromere_region[1] else 0
        )
        ...
        features["distance_to_nearest_telomere_normalized"] = (
            features["distance_to_nearest_telomere"] / chr_len
        )
    ...
    def extract_structural_features(self, ref: str, alt: str, effect: str) -> Dict:
        ...
        features["gc_content_change"] = (
            features["alt_gc_content"] - features["ref_gc_content"]
        )
        ...
        features["is_lof"] = (
            ...
            if any(
                x in effect_lower
                for x in ["frameshift", "nonsense", "stop_gained", "splice"]
            )
            else 0
        )
    ...
    def extract_signature_features(
        self, mut_type: str, mut_subtype: str, ref: str, alt: str, chr_str: str
    ) -> Dict:
    ...
    def extract_mutation_context_features(
        ...
        ref: str,
        alt: str,
        all_mutations_df: pd.DataFrame = None,
    ) -> Dict:
        ...
        same_chr_filtered = same_chr[
            same_chr["distance"] > 0
        ]  # Exclure mutation elle-même

        # Comptages dans fenêtres
        features["n_mutations_100kb"] = len(
            same_chr_filtered[same_chr_filtered["distance"] <= 100000]
        )
        features["n_mutations_1mb"] = len(
            same_chr_filtered[same_chr_filtered["distance"] <= 1000000]
        )
        features["n_mutations_10mb"] = len(
            same_chr_filtered[same_chr_filtered["distance"] <= 10000000]
        )

        # Densité locale
        features["mutation_density_local"] = (
            features["n_mutations_100kb"] / 100
        )  # Par 100kb
        ...
        if len(same_chr_filtered) > 0:
            ...
            features["log_min_distance"] = np.log1p(
                features["min_distance_to_other_mut"]
            )
        else:
    ...
    def extract_all_features(
        self, mutations_df: pd.DataFrame, include_context: bool = True
    ) -> pd.DataFrame:
        """
        ...
        if include_context:
            ...
            for col in ["chr", "start"]:
                if col in context_df.columns:
                    context_df[col] = (
                        context_df[col].astype(str)
                        if col == "chr"
                        else pd.to_numeric(context_df[col], errors="coerce")
                    )
        else:
            context_df = None
        ...
        for idx, row in mutations_df.iterrows():
            if idx % 500 == 0:
                print(
                    f"  Progression: {idx}/{len(mutations_df)} ({idx / len(mutations_df) * 100:.1f}%)"
                )
            ...
            sig_feat = self.extract_signature_features(
                mut_type, mut_subtype, ref, alt, chr_str
            )
            features.update(sig_feat)

            # 8. Features contexte mutationnel (optionnel)
            if include_context and context_df is not None:
                context_feat = self.extract_mutation_context_features(
                    chr_str, start, ref, alt, context_df
                )
