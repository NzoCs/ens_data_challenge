...
class FeatureEngineerHelper:
    ...
    def Nmut(self, molecular_data: pd.DataFrame, clinical_data: pd.DataFrame) -> pd.DataFrame:
        """
        ...
        clinical_data = clinical_data.merge(tmp, on='ID', how='left').fillna({'Nmut': 0})
    ...
    def ratios_and_interactions(self, clinical_data: pd.DataFrame) -> pd.DataFrame:
        ...
        clinical_data['blast_cyto_complexity'] = clinical_data['BM_BLAST'] * clinical_data['n_abnormalities']
        # Charge tumorale composite: (BM_BLAST + WBC_log) × risque_cyto
        ...
        clinical_data['tumor_burden_composite'] = (clinical_data['BM_BLAST'] + wbc_log) * cyto_risk_num
    ...
    def severity_scores(self, clinical_data: pd.DataFrame) -> pd.DataFrame:
        ...
        clinical_data['cytopenias_count'] = clinical_data.apply(count_cytopenias, axis=1)
    ...
    def _pathways_classification(
        ...
        molecular_data['PATHWAY'] = molecular_data['GENE'].apply(FeatureEngineerHelper._map_gene_to_pathway())
    ...
    def _create_confidence_weighted_count_matrix(
        ...
        df_filtered = molecular_data[molecular_data[col].isin(categories_to_include)].copy()
    ...
    def add_mol_encoding(
        ...
        Returns:
            List[str]: noms des colonnes gènes ajoutées dans `self.clinical_data`/`self.clinical_test`.
        """
        molecular_data = molecular_data.copy()
