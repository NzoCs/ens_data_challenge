...
from .types import CytogeneticsPatterns
...
class ParsedKaryotype:
    ...
    def is_normal(self) -> bool:
        ...
        return (
            ...
            and not self.translocations
            and not self.inversions
            and not self.derivatives
            and not self.duplications
            and not self.triplications
            and not self.isochromosomes
            and not self.markers
            and not self.monosomies
            and not self.trisomies
        )
...
class CytogeneticsParser:
    ...
    def parse(self, cyto_string: Any) -> Optional[List[ParsedKaryotype]]:
        ...
        if special_obj is not None:
            ...
            if type_name not in self.seen_special_types:
                self.logger.info(
                    f"Encountered new special cytogenetic type: {type_name}"
                )
    ...
    def _is_normal_karyotype(self, cyto: str) -> bool:
        ...
        return bool(
            re.match(CytogeneticsPatterns.NORMAL_KARYOTYPE, cyto, re.IGNORECASE)
        )
    ...
    def _identify_special_case(self, cyto_str: str) -> Optional[ParsedKaryotype]:
        ...
        def _get_dummy_complex(n=3):
            return [
                {"raw": f"der({i + 1})", "chromosome": str(i + 1)} for i in range(n)
            ]
    ...
    def _extract_deletions(self, cyto: str) -> List[dict]:
        ...
        matches = re.findall(
            CytogeneticsPatterns.DELETION_POSITIONS, cyto, re.IGNORECASE
        )
        if matches:
    ...
    def _extract_additions(self, cyto: str) -> List[dict]:
        ...
        matches = re.findall(
            CytogeneticsPatterns.ADDITION_POSITIONS, cyto, re.IGNORECASE
        )
        if matches:
    ...
    def _extract_translocations(self, cyto: str) -> List[dict]:
        ...
        matches = re.findall(
            CytogeneticsPatterns.TRANSLOCATION_POSITIONS, cyto, re.IGNORECASE
        )
        if matches:
        ...
        for chr1, chr2 in matches:
            translocations.append(
                {"type": "translocation", "chromosomes": (chr1, chr2)}
            )
    ...
    def _extract_inversions(self, cyto: str) -> List[dict]:
        ...
        matches = re.findall(
            CytogeneticsPatterns.INVERSION_POSITIONS, cyto, re.IGNORECASE
        )
        if matches:
            for chr_num, start_arm, start_band, end_arm, end_band in matches:
                data = {
    ...
    def _extract_derivatives(self, cyto: str) -> List[dict]:
        ...
        matches = re.findall(
            CytogeneticsPatterns.DERIVATIVE_POSITIONS, cyto, re.IGNORECASE
        )
        if matches:
    ...
    def _extract_duplications(self, cyto: str) -> List[dict]:
        ...
        matches = re.findall(
            CytogeneticsPatterns.DUPLICATION_POSITIONS, cyto, re.IGNORECASE
        )
        if matches:
            for chr_num, start_arm, start_band, end_arm, end_band in matches:
                data = {
    ...
    def _extract_triplications(self, cyto: str) -> List[dict]:
        ...
        matches = re.findall(
            CytogeneticsPatterns.TRIPLICATION_POSITIONS, cyto, re.IGNORECASE
        )
        if matches:
            for chr_num, start_arm, start_band, end_arm, end_band in matches:
                data = {
    ...
    def _extract_isochromosomes(self, cyto: str) -> List[dict]:
        ...
        matches = re.findall(
            CytogeneticsPatterns.ISOCHROMOSOME_POSITIONS, cyto, re.IGNORECASE
        )
        if matches:
